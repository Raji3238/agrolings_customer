...
app.get('/postData', (req, res) => {
  ...
  connection.query(getCustomerQuery,function(err,customer){
    ...
    res.send(err);
      ...
      var insertQuery = "insert into client (name,client_address,phone_number) values('"+parsedData.customerName+"','"+parsedData.address+"',"+parsedData.mobileNumber+")";
      console.log('insert query',insertQuery);
      connection.query(insertQuery,function(err,saveClient){
        console.log('insertt',err,saveClient)
        if(err) res.send('error');
        else{
          //get customer id
         
          saveOrderDetails(parsedData);
        }
      })
    ...
    function saveOrderDetails(){
      ...
      var getCustomers = "select id from client where phone_number="+parsedData.mobileNumber;
      connection.query(getCustomers,function(err,customer){
        //customer.push({id:saveClient[0].id})
